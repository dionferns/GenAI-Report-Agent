...
> A production-grade, agentic news intelligence system built for the Data Reply Mid-Senior AI Engineer assessment.
> Designed to be evaluated, observed, and deployed — not just demonstrated.
...
6. [Ingestion Graph — Node-by-Node Spec](#6-ingestion-graph--node-by-node-spec)
7. [Chat Graph — Node-by-Node Spec](#7-chat-graph--node-by-node-spec)
...
13. [Streamlit UI Spec](#13-streamlit-ui-spec)
14. [Scheduler](#14-scheduler)
15. [CI/CD Pipeline](#15-cicd-pipeline)
16. [Environment Variables](#16-environment-variables)
17. [Quickstart](#17-quickstart)
18. [AWS Deployment Notes](#18-aws-deployment-notes)
...
1. **Automatically ingests** articles from BBC News and UK Government (gov.uk) every hour, focused on the topic of **UK AI Regulation**.
2. **Processes and stores** articles in a persistent vector store with deduplication — so hourly runs accumulate a growing, clean corpus rather than polluting it with repeated content.
3. **Generates a structured report** every hour containing a 100–150 word summary paragraph, 3–5 key takeaways, mentioned organisations and key terms, and a delta noting what is new compared to the previous hour's report.
4. **Provides a conversational interface** (Streamlit) where users can ask specific, vague, or adversarial questions. The system routes intelligently, retrieves relevant context from the full corpus using hybrid search (BM25 + vector), and responds with inline citations.
5. **Is fully observable** via LangSmith tracing and structured JSON logging with `structlog`.
6. **Is continuously evaluated** via a RAGAS evaluation harness run against a hand-crafted golden dataset, with results committed to the repo.

### Why This Architecture

The brief asked for a news summariser. This system was built as if a real client at Data Reply had requested it for production use. That means it addresses concerns a graduate submission typically ignores:

- **Hallucination risk** — a Critic node in the ingestion graph checks every report against its source chunks before it is persisted.
- **Data quality over time** — hourly deduplication prevents vector store degradation.
- **Explainability** — every chat response cites specific source URLs and retrieval timestamps.
- **Adversarial users** — guardrails on the chat input detect prompt injection and off-topic queries.
- **Evaluability** — RAGAS metrics are tracked over time, not just run once for the demo.
